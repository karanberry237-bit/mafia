...
async function acceptBribe(targetUserId, addCopper) {
  ...
  if (leaveError) {
    ...
    await addCopper(offer.offeredBy, offer.amount); // refund — the move failed
  ...
  if (joinError) {
    ...
    await addCopper(offer.offeredBy, offer.amount); // refund — you didn't actually end up in their gang
